...
def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    pos_enc_dim = cfg.pos_enc.lap_dim
    ...
    if 'LapPE' in pe_types or 'EquivStableLapPE' in pe_types:
        ...
        EigVal, EigVec = get_lap_decomp_stats(
        ...
        print(EigVec.shape)
        pos_enc = EigVec[:, 1:pos_enc_dim+1].float()
        print(pos_enc.shape)
        if pos_enc.size(1) < pos_enc_dim:
            zeros = torch.zeros(data.num_nodes, pos_enc_dim)
