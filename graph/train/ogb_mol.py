    ...
    for step, batch in enumerate(tqdm(train_loader, desc="Iteration")):
        ...
        if batch.x.shape[0] == 1 or batch.batch[-1] == 0:
            ...
            if "classification" in task_type: 
                loss = cls_criterion(pred.to(torch.float32)[is_labeled], batch.y.to(torch.float32)[is_labeled]) + link_loss
            else:
                loss = reg_criterion(pred.to(torch.float32)[is_labeled], batch.y.to(torch.float32)[is_labeled]) + link_loss
            loss.backward()
            optimizer.step()
