...
def eval_ap(y_true, y_pred):
    '''
        compute F1 score
    '''
    f1 = torchmetrics.F1Score(task = "multiclass", num_classes = 21, average = "macro")
    y_true = y_true.cpu().detach()
    y_pred = y_pred.cpu().detach().argmax(-1)
...
def train(train_loader, model, optimizer, evaluator, device, sharp):
    ...
    for _, data in tqdm(enumerate(train_loader)):
        ...
        # loss = criterion(out[mask], y[mask].long())[0] + 5*loss_cluster
