...
from torch_geometric.nn import MLP, PointConv, knn
...
class GatherLocalInformation(torch.nn.Module):
    ...
    def forward(self, pxo):
        p, x, o = pxo
        row, col = knn(p, p, self.n, o, o)
        edge_index = torch.stack([col, row], dim=0)
        x = self.conv((x, x), (p, p), edge_index)
        return x
...
class Init(nn.Module):
        ...
        for i in range(0, self.depth):
            if i == 0:
                self.projections.append(
                    GatherLocalInformation(16, MLP([in_channel + 3, self.dim*expand**i]))
                )
            else:
                self.projections.append(
                    nn.Sequential(
    ...
    def forward(self, pxbim):
        ...
        for i, ii in enumerate(idx):
            if i == 0:
                x_proj = self.projections[i]([p[ii.long()], x_proj[ii.long()], b[ii.long()]])
            else:
...
class TreeTransformerCls(nn.Module):
    def __init__(self, args):
        super().__init__()

        
        self.n = args.num_point
        self.c = args.c
        self.dim = args.dim
    ...
    def _pooling(self, x_c, idx, o, selection = "root", method = "average"):
        if selection == "root":
            return x_c[-1]
        elif selection == "inner":
            idxi = idx[-2]
            x = x_c[-2]
            out = torch.zeros(o.shape[0], x.shape[-1]).type_as(x)
            prev = 0
            for i in range(o.shape[0]):
                x_temp = x[(prev <= idxi) & (idxi < o[i])]
                out[i] = x_temp.mean(0)
                prev = o[i]
            return out
        elif selection == "leaf":
            x = x_c[0]
            x = x.reshape(o.shape[0], -1, x.shape[-1])
            return x.mean(1)
    ...
    def forward(self, pxom):
        p, x, o, m = pxom  # (n, 3), (n, c), (b)
        length = x.shape[0]

        m_org = m.clone()

        b = torch.zeros(p.shape[0]).to(p.device)
        begin = 0
        ...
        x = self._pooling(x_c, idx, o, self.selection)
